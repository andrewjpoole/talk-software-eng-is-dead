...
class SimpleAnimateSvgComponent extends HTMLElement {
  ...
  // ─── Colour inversion via OKLCH lightness flip ───────────────────────────────
  // Inverts L in Oklab (L → 1-L) while preserving chroma and hue.
  // This means black→white, mid-grey stays mid-grey, and coloured elements
  // keep their hue/saturation but flip between light and dark.

  _invertColors(svg) {
    svg.querySelectorAll('path,line,polyline,polygon,rect,circle,ellipse,text,tspan').forEach(el => {
      const cs = window.getComputedStyle(el);
      const elOp = parseFloat(cs.opacity) || 1;
      this._invertByOklch(el, 'stroke', cs.stroke, elOp * (parseFloat(cs.strokeOpacity) || 1));
      this._invertByOklch(el, 'fill',   cs.fill,   elOp * (parseFloat(cs.fillOpacity)   || 1));
    });
  }

  _invertByOklch(el, prop, color, effectiveAlpha = 1) {
    if (!color || color === 'none') return;
    const c = this._parseRgba(color);
    if (!c || c.a === 0) return;
    const [r, g, b] = this._oklchInvert(c.r / 255, c.g / 255, c.b / 255, effectiveAlpha * c.a);
    const clamp = v => Math.round(Math.max(0, Math.min(1, v)) * 255);
    el.style[prop] = c.a < 1
      ? `rgba(${clamp(r)},${clamp(g)},${clamp(b)},${c.a})`
      : `rgb(${clamp(r)},${clamp(g)},${clamp(b)})`;
  }

  _oklchInvert(r, g, b, effectiveAlpha = 1) {
    // sRGB → linear sRGB
    const lin = v => v <= 0.04045 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4;
    const rl = lin(r), gl = lin(g), bl = lin(b);

    // Linear sRGB → Oklab (Björn Ottosik 2020)
    ...
    // Semi-transparent bright colours act as "glow overlays" — they work on dark
    // backgrounds too, so preserve L rather than inverting to near-invisible dark.
    const L2 = (effectiveAlpha < 0.5 && L > 0.5) ? L : 1 - L;

    // Oklab → linear sRGB
    const l2 = (L2 + 0.3963377774*a + 0.2158037573*bk) ** 3;
    const m2 = (L2 - 0.1055613458*a - 0.0638541728*bk) ** 3;
    const s2 = (L2 - 0.0894841775*a - 1.2914855480*bk) ** 3;
    const rl2 =  4.0767416621*l2 - 3.3077115913*m2 + 0.2309699292*s2;
    const gl2 = -1.2684380046*l2 + 2.6097574011*m2 - 0.3413193965*s2;
    const bl2 = -0.0041960863*l2 - 0.7034186147*m2 + 1.7076147010*s2;

    // Linear sRGB → sRGB (out-of-gamut values are clamped by caller)
    const srgb = v => v <= 0.0031308 ? 12.92 * v : 1.055 * v ** (1 / 2.4) - 0.055;
    return [srgb(rl2), srgb(gl2), srgb(bl2)];
  }

  _parseRgba(c) {
    // Handles rgb(...) and rgba(...) as returned by getComputedStyle
    const m = c.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\s*\)/i);
    if (m) return { r:+m[1], g:+m[2], b:+m[3], a: m[4] !== undefined ? +m[4] : 1 };
    // Hex fallback (#rgb #rrggbb #rrggbbaa)
    const h = c.match(/^#([0-9a-f]{3,8})$/i)?.[1];
    if (h) {
      const x = h.length <= 4
      ...
      return {
        ...
        b: parseInt(x.slice(4,6),16), a: parseInt(x.slice(6,8),16) / 255
      };
    }
    return null;
  }
...
if (!customElements.get('simple-animate-svg-component-v3')) {
  customElements.define('simple-animate-svg-component-v3', SimpleAnimateSvgComponent);
}
